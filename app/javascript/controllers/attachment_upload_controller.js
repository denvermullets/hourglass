...
export default class extends Controller {
  ...
  _renderPreviewStrip() {
    ...
    this.previewStripTarget
      .querySelectorAll("[data-pending-file], [data-add-more]")
    ...
    // Add "+" button
    this.previewStripTarget.insertAdjacentHTML(
      "beforeend",
      `
      <button type="button" data-action="click->attachment-upload#openFilePicker" data-add-more
              class="flex items-center justify-center w-13 h-13 border border-dashed border-bunker-825 rounded cursor-pointer shrink hover:border-bunker-700 transition-colors duration-150">
  ...
  _imageThumbHtml(entry) {
    ...
    return `
      <div class="w-13 h-13 rounded shrink relative overflow-hidden border border-bunker-825 bg-bunker-875" data-pending-file>
  ...
  _fileThumbHtml(entry) {
    ...
    return `
      <div class="w-13 h-13 rounded shrink relative border border-bunker-825 bg-bunker-875 flex flex-col items-center justify-center gap-0.5" data-pending-file>
  ...
  _showError(message) {
    ...
    toast.className =
      "fixed bottom-4 left-1/2 -translate-x-1/2 bg-danger-800 text-danger-400 text-xs font-dm-mono px-4 py-2 rounded border border-danger-800 z-50";
